...
(function (global, factory) {
  ...
  var svg_to_fold = function svg_to_fold(svg) {
    var graph = emptyFOLD();
    var v0 = graph.vertices_coords.length;
    var segments = Segmentize(svg);
    graph.vertices_coords = segments.map(function (s) {
      return [[s[0], s[1]], [s[2], s[3]]];
    }).reduce(function (a, b) {
      return a.concat(b);
    }, []);
    graph.edges_vertices = segments.map(function (_, i) {
      return [v0 + i * 2, v0 + i * 2 + 1];
    });
    graph.edges_assignment = segments.map(function (l) {
  ...
  var SVGtoFOLD = function SVGtoFOLD(input, options) {
